...
<?php
    ...
    if(!empty($modalType) ){
        ...
        $sql_total = "SELECT COUNT(1) FROM `product` WHERE `product_model` = '$modalType' AND `product_out` = '0' AND `product_email` = '$e_mail'";
        ...
        $sql = "SELECT * FROM `product` WHERE `product_model` = '$modalType' AND `product_out` = '0' AND `product_email` = '$e_mail'";
        $stmt = $pdo->query($sql);
    ...
    elseif(!empty($brand) ){
        $sql_total = "SELECT COUNT(1) FROM `product` WHERE `product_brand` = '$brand' AND `product_out` = '0' AND `product_email` = '$e_mail'";
        ...
        $sql = "SELECT * FROM `product` WHERE `product_brand` = '$brand' AND `product_out` = '0' AND `product_email` = '$e_mail'" ;
        $stmt = $pdo->query($sql);
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    elseif(!empty($quantity) ){

        $sql_total = "SELECT COUNT(1) FROM `product` WHERE `product_quantity` < '$quantity' AND `product_out` = '0' AND `product_email` = '$e_mail'";
        $totalRows = $pdo->query($sql_total)->fetch(PDO::FETCH_NUM)[0];
        $totalPages = ceil( $totalRows/$perPage);
        ...
        $sql = "SELECT * FROM `product` WHERE `product_quantity` < '$quantity' AND `product_out` = '0' AND `product_email` = '$e_mail'" ;
        $stmt = $pdo->query($sql);
    ...
    elseif(!empty($cutaway) ){
        ...
        $sql_total = "SELECT COUNT(1) FROM `product` WHERE `product_cutaway` = '$cutaway' AND `product_out` = '0' AND `product_email` = '$e_mail'";
        $totalRows = $pdo->query($sql_total)->fetch(PDO::FETCH_NUM)[0];
        $totalPages = ceil( $totalRows/$perPage);
        ...
        $sql = "SELECT * FROM `product` WHERE `product_cutaway` = '$cutaway' AND `product_out` = '0' AND `product_email` = '$e_mail'";
        $stmt = $pdo->query($sql);
        ...
        $sql_total = "SELECT COUNT(1) FROM `product` WHERE `product_out` = '0' AND `product_email` = '$e_mail'";
        // $sql_total = sprintf("SELECT COUNT(1) FROM `product` WHERE `product_email`= %s", $_SESSION['loginUser']['email']);
        //執行SQL語法並 拿到總比數
        $totalRows = $pdo->query($sql_total)->fetch(PDO::FETCH_NUM)[0];
        // 有幾頁 = 總比數/每一頁顯示幾筆
        ...
        $sql = "SELECT * FROM `product` WHERE `product_out` = '0' AND `product_email` = '$e_mail'" ;
        $stmt = $pdo->query($sql);
...
<script>
       ...
       $('#product-table').dataTable({
        ...
        function selectCategory(element_id, selection){
            ...
            window.location.href= `product_up.php?${selection}=${selectValue}`;
        }


        function lowQuantity(column, quantity){
            window.location.href= `product_up.php?${column}=${quantity}`;
        }

        function showCutaway(){
            window.location.href= `product_up.php?cutaway=1`;
        }
